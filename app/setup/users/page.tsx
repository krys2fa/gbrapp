  ...
  React.useEffect(() => {
    async function fetchUsers() {
      ...
      try {
        ...
        if (emailFilter) params.push(`search=${encodeURIComponent(emailFilter)}`);
        ...
        <div className="max-w-md mx-auto">
          ...
          {editingUser && (
            <button
              ...
              onClick={() => { setEditingUser(null); setForm({ name: "", email: "", password: "", role: roles[0] }); }}
            >Cancel Edit</button>
          )}
          ...
          {usersLoading ? (
            ...
            <>
              <table className="w-full divide-y divide-gray-200">
                ...
                <tbody>
                  {users.slice((page-1)*pageSize, page*pageSize).map((user: any) => (
                    <tr key={user.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 text-gray-900">{user.name}</td>
                      <td className="px-4 py-2 text-gray-700">{user.email}</td>
                      <td className="px-4 py-2 text-gray-700">{user.role}</td>
                      <td className="px-4 py-2 text-gray-700">{user.isActive ? "Active" : "Inactive"}</td>
                      <td className="px-4 py-2 text-gray-700">{user.createdAt ? new Date(user.createdAt).toLocaleDateString() : "-"}</td>
              ...
              <div className="flex justify-between items-center mt-4">
                <span className="text-sm text-gray-600">Page {page} of {Math.max(1, Math.ceil(users.length/pageSize))}</span>
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
    <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative">
      ...
      <div className="space-y-2">
        ...
        <div><span className="font-semibold">Role:</span> {viewingUser.role ? viewingUser.role.charAt(0) + viewingUser.role.slice(1).toLowerCase() : ""}</div>
        <div><span className="font-semibold">Status:</span> {viewingUser.isActive ? "Active" : "Inactive"}</div>
        <div><span className="font-semibold">Date Added:</span> {viewingUser.createdAt ? new Date(viewingUser.createdAt).toLocaleDateString() : "-"}</div>
        <div><span className="font-semibold">Last Login:</span> {viewingUser.lastLogin ? new Date(viewingUser.lastLogin).toLocaleString() : "Never"}</div>
      </div>
    </div>
  </div>
)}
