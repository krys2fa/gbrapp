...
export default function DailyPricesPage() {
  ...
  const handleAddExchange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message || "Error adding exchange");
    } finally {
  ...
  const handleEdit = (priceObj: any) => {
    ...
    if (priceObj.type === "COMMODITY") {
      ...
      setCommodityDate(priceObj.date ? new Date(priceObj.date).toISOString().split('T')[0] : (() => {
        const today = new Date();
        return today.toISOString().split('T')[0];
      })());
      ...
      setExchangeDate(priceObj.date ? new Date(priceObj.date).toISOString().split('T')[0] : (() => {
        const today = new Date();
        return today.toISOString().split('T')[0];
      })());
    }
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="date"
                  value={commodityDate}
                  onChange={(e) => setCommodityDate(e.target.value)}
                  className="mt-1 block w-full border px-3 py-2 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                  required
                />
              </div>
