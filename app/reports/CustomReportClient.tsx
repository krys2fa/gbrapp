    ...
    <div className="bg-white rounded-2xl p-6 border border-gray-200 mt-6">
      ...
      >
        <select
          ...
          onChange={(e) =>
            setFilters({ ...filters, exporterId: e.target.value })
          }
        ...
        <input
          placeholder="Min net gold (g)"
          value={filters.minNetGold}
          onChange={(e) =>
            setFilters({ ...filters, minNetGold: e.target.value })
          }
          className="p-2 border rounded"
        />
        <input
          placeholder="Max net gold (g)"
          value={filters.maxNetGold}
          onChange={(e) =>
            setFilters({ ...filters, maxNetGold: e.target.value })
          }
          className="p-2 border rounded"
        />
        <label className="inline-flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              setFilters({ ...filters, includeFees: e.target.checked })
            }
          />{" "}
          <span className="ml-1">Include Fees</span>
        </label>
        ...
        <div className="sm:col-span-3 flex gap-2">
          ...
          <a
            ...
            href={`/api/reports/custom?${buildQuery({
              ...filters,
              page: 1,
              limit,
              includeFees:
                filters.includeFees || selectedColumns.includes("feesTotal"),
              columns: selectedColumns,
            })}&download=csv`}
          >
            Download CSV
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {availableColumns.map((col) => (
            <label
              key={col.key}
              className="inline-flex items-center gap-2 p-1 border rounded text-sm"
            >
              <input
                className="w-4 h-4"
                type="checkbox"
                ...
                onChange={(e) => {
                  ...
                  if (col.key === "feesTotal" && checked)
                    setFilters((f: any) => ({ ...f, includeFees: true }));
                }}
              />
              {col.label}
            </label>
      ...
      <div className="mb-3 text-sm text-gray-600">
        {loading ? "Loading..." : `Showing ${rows.length} of ${total} results`}
      </div>
                  ...
                  const value = (() => {
                    switch (col) {
                      ...
                      case "jobCardId":
                        return r.jobCardId || "";
                      case "netGoldGrams":
                        return typeof r.netGoldGrams === "number"
                          ? r.netGoldGrams.toFixed(2)
                          : "";
                      case "netSilverGrams":
                        return typeof r.netSilverGrams === "number"
                  ...
                  return (
                    <td
                      ...
                      className={`px-4 py-2 text-sm ${
                        col === "exporter" ? "font-medium text-indigo-600" : ""
                      } ${
                        col === "netGoldGrams" ||
                        col === "netSilverGrams" ||
                        col === "estimatedValue" ||
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() => {
              ...
              fetchData({ page: Math.max(1, page - 1) });
            }}
          >
            Prev
          </button>
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setPage(1);
              fetchData({ page: 1, limit: Number(e.target.value) });
            }}
      ...
      <div className="mt-6">
        <h4 className="text-sm font-medium mb-2">
          Net Gold by Exporter (current page)
        </h4>
        <div className="flex items-end gap-2 h-40">
          {(() => {
            ...
            for (const r of rows)
              groups.set(
                r.exporter,
                (groups.get(r.exporter) || 0) + r.netGoldGrams
              );
            ...
            return entries.map(([k, v]) => (
              ...
              >
                <div
                  style={{ height: `${(v / max) * 100}%`, width: 24 }}
                  className="bg-amber-400 w-6 rounded-t"
                ></div>
                <div className="text-xs mt-1 text-center truncate w-12">
                  {k}
                </div>
              </div>
            ));
