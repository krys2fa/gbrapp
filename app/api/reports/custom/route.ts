...
export async function GET(req: Request) {
  ...
  const minNetGold = url.searchParams.get("minNetGold") ? Number(url.searchParams.get("minNetGold")) : undefined;
  const maxNetGold = url.searchParams.get("maxNetGold") ? Number(url.searchParams.get("maxNetGold")) : undefined;
  const includeFees = url.searchParams.get("includeFees") === "1" || url.searchParams.get("includeFees") === "true";
  const download = url.searchParams.get("download") === "csv";
  const columnsParam = url.searchParams.get("columns") || undefined;
  const requestedColumns = columnsParam ? columnsParam.split(",").map((s) => s.trim()).filter(Boolean) : undefined;
  ...
  const responseRows = await Promise.all(rows.map(async (c: any) => {
    const r = c.jc;
    ...
    if (r.fees && r.fees.length) {
      ...
      feesTotal = fees.reduce((s: number, f: any) => s + Number(f.amountPaid ?? f.amount ?? 0), 0);
      whtTotal = fees.reduce((s: number, f: any) => s + Number(f.whtTotal ?? 0), 0);
    ...
    let certificateNumber = r.assays && r.assays.length ? r.assays[0].certificateNumber : undefined;
    let assayDate = r.assays && r.assays.length ? r.assays[0].assayDate : undefined;
    if (r.assays && r.assays.length > 1) {
      // try to pick the most recent assay
      const sorted = [...r.assays].sort((a: any, b: any) => new Date(b.assayDate).getTime() - new Date(a.assayDate).getTime());
    ...
    const totalNetWeightOz = Number(r.totalNetWeightOz ?? (c.netGoldGrams / GRAMS_PER_TROY_OUNCE));
    const goldOunces = c.netGoldGrams / GRAMS_PER_TROY_OUNCE;
    const valuePerGram = c.netGoldGrams ? (estimatedValue / c.netGoldGrams) : 0;

    const daysSinceReceived = r.receivedDate ? Math.floor((Date.now() - new Date(r.receivedDate).getTime()) / (1000 * 60 * 60 * 24)) : null;
  ...
  if (download) {
    const defaultOrder = ["createdAt", "exporter", "jobCardId", "netGoldGrams", "netSilverGrams", "estimatedValue", "feesTotal"];
    const colOrder = requestedColumns && requestedColumns.length ? requestedColumns : defaultOrder;
    ...
    for (const r of responseRows) {
      const rowValues = colOrder.map((c) => {
        switch (c) {
          case "createdAt":
            return csvEscape(r.createdAt ? new Date(r.createdAt).toISOString() : "");
          case "exporter":
            return csvEscape(r.exporter);
          case "jobCardId":
            return csvEscape(r.jobCardId);
          case "netGoldGrams":
            return csvEscape(typeof r.netGoldGrams === 'number' ? r.netGoldGrams.toFixed(2) : r.netGoldGrams);
          case "netSilverGrams":
            return csvEscape(typeof r.netSilverGrams === 'number' ? r.netSilverGrams.toFixed(2) : r.netSilverGrams);
          case "estimatedValue":
            return csvEscape(typeof r.estimatedValue === 'number' ? r.estimatedValue.toFixed(2) : r.estimatedValue);
          case "feesTotal":
            return csvEscape(typeof r.feesTotal === 'number' ? r.feesTotal.toFixed(2) : r.feesTotal);
          default:
    ...
    return new Response(lines.join("\n"), { status: 200, headers: { "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": `attachment; filename="custom-report-${Date.now()}.csv"` } });
  }

  return new Response(JSON.stringify({ rows: responseRows, total }), { status: 200, headers: { "Content-Type": "application/json" } });
}
